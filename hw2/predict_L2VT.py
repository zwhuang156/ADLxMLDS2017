...
def Load_testing_data(test_data_path, vocab_size):
    ...
    lst_in = np.zeros((Y.shape[0],Y.shape[1],Y.shape[2]))
    print(X_test.shape)

    return X_test, Y, filename ,lst_in
...
if __name__ == '__main__':
    ...
    test_data_path = "data/testing_data/feat"
    train_label_path = "data/training_label.json"
    ans_path = "ans/ans_L2VT.csv"
    vocab_size = 3000
    ...
    X_test, Y, filename, lst_in = Load_testing_data(test_data_path, vocab_size)
    model = load_model(model_path, custom_objects={'masked_categorical_crossentropy': mask_categorical_crossentropy})
    result = model.predict([X_test, lst_in, Y])
