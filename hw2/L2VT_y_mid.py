...
def Load_train_data(filelist, train_data_path, train_label_path, vocab_dict, max_seq_length, unit):
    ...
    for file in filelist:
        ...
        pad = np.zeros((80, unit))
        for i in range(size):
            X_train.append(feature)
            mid_padding.append(pad)
        
    X_train = np.array(X_train)
    Y_train_in = np.array(Y_train_in)
    Y_train_out = np.array(Y_train_out)
    mid_padding = np.array(mid_padding)
    
    lst_in = np.zeros((Y_train_in.shape[0],Y_train_in.shape[1],Y_train_in.shape[2]))
    ...
    """
    return X_train, Y_train_in, Y_train_out, mid_padding, lst_in
...
if __name__ == '__main__':
    ...
    '''
    padding = Input(shape=(80, unit_1st))
    concate_1 = concatenate([en_1_out, padding], axis=-1)
    '''
    ...
    mid_input = Input(shape=(1, vocab_size))
    mid = mid_input
    for i in range(max_seq_length-1) :
        ...
        concate_2 = concatenate([de_1_out, mid], axis=-1)
        
        de_2_out, de_2_state_h, de_2_state_c = LSTM(unit_1st, return_sequences=True, return_state=True, activation='relu')(concate_2, initial_state=state_2)
    ...
    model = Model([en_input,de_input,mid_input], final_outputs)
    ...
    for i in range(epochs):
        ...
        for file in all_file:
            ...
            if count%batch_size==0:
                X_train,Y_train_in,Y_train_out,mid_padding,lst_in = Load_train_data(filelist, path.train_data_path, path.train_label_path, OneHot_table, max_seq_length, unit_1st)
                filelist = []
                model.fit([X_train,lst_in,Y_train_in], Y_train_out, epochs = 1, verbose=1)
                del X_train, Y_train_in, Y_train_out,mid_padding,lst_in
