...
from keras.layers import LSTM, Conv2D, MaxPooling2D, Flatten, Masking, Dense, Dropout, Activation, BatchNormalization, Bidirectional, TimeDistributed
...
class Path_define():
    ...
    model_path = "model/" + TrainSet + "_model.h5"
...
def Reconstruct_and_Padding_X(data):
    new_data = np.zeros((3696,777,data.shape[1]-1))    
    sentence = 0
    frame = 0
...
def Reconstruct_and_Padding_Y(data):
    new_data = np.zeros((3696,777,48),dtype=np.int)    
    sentence = 0
    frame = 0
    for i in range(len(data)):
        now = data[i][0].split("_")
...
def Reconstruct_and_Padding_X_for_conv2D(data):
    new_data = np.zeros((3696,777,data.shape[1]-1,1))  
    sentence = 0
    frame = 0
    for i in range(len(data)):
        now = data[i][0].split("_")
